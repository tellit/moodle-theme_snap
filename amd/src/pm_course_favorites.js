    ...
    function($, ajax, notification, log, mview, ajaxNotify) {
        return function() {
            ...
            var moveOutOfFavorites = function(cardEl, onMoveComplete) {
                ...
                // When this is 0 we only have hidden courses, so container is #cass-pm-courses-current-cards.
                var publishedcount = $('#cass-pm-courses-current .coursecard:not([data-hidden="true"])').length;
                // Special stuff for when moving a hidden course.
                if ($(cardEl).data('hidden') === true && publishedcount > 0) {
                    container = '#cass-pm-courses-hidden-cards';
                    // Open hidden courses section.
                    $('#cass-pm-courses-hidden').addClass('state-visible');
                    $('#cass-pm-courses-hidden-cards').collapse('show');
                } else {
                    window.console.log('not a hidden card');
                    container = '#cass-pm-courses-current-cards';
                }
            ...
            var favoriteCourse = function(button) {
                ...
                if (favorited === 1) {
                    ...
                    // Move to favorites.
                    moveCard(cardEl, '#cass-pm-courses-current-cards .coursecard.favorited', '#cass-pm-courses-current-cards', true,
                        function() {
                            doAjax(jsid);
                        }
                    );
             ...
             */
            $("#cass-pm").on("click", ".favoritetoggle", function(e) {
                e.preventDefault();
                e.stopPropagation();
                favoriteCourse(this);
            });
