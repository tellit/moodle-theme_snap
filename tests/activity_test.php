...
use \theme_cass\activity,
    \theme_cass\cass_base_test;

global $CFG;
require_once($CFG->dirroot . '/mod/assign/tests/base_test.php');

/**
 * Testing for theme/cass/classes/activity.php
 *
 * @package  theme_cass
 * @copyright  2017 Blackboard inc.
 * @license  http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_cass_acitvity_test extends cass_base_test {
    ...
    public function test_get_calendar_activity_events() {
        ...
        $cassevent = reset($events);
        $this->assertEquals($due, $cassevent->timestart);
        $this->assertEquals($due, $cassevent->timesort);

        // Test extension set.
        $this->extend_assign_deadline($assign->id, $student->id, $extension);
        // Test that the Cass calendar includes extensions to assignments.
        $events = activity::get_calendar_activity_events($tstart, $tend, $courses);
        $cassevent = reset($events);
        $this->assertEquals($extension, $cassevent->timestart);
        $this->assertEquals($extension, $cassevent->timesort);
        ...
        // Test that the Cass calendar event is the same as the core event
        // (with the exception of the timestart + timesort fields).
        unset($cassevent->timestart);
        unset($cassevent->timesort);
        unset($coreevent->timestart);
        unset($coreevent->timesort);
        $this->assertEquals($cassevent, $coreevent);
    }
    ...
    public function test_user_activity_events() {
        ...
        // Assert not from cache.
        $this->assertFalse($eventsobj->fromcache);
        $this->assertEquals($ovdgroupdue, $cassevent->timestart);
        $this->assertEquals($ovdgroupdue, $cassevent->timesort);
        ...
        // Assert not from cache.
        $this->assertFalse($eventsobj->fromcache);
        $this->assertEquals($ovduserdue, $cassevent->timestart);
        $this->assertEquals($ovduserdue, $cassevent->timesort);
    ...
    public function test_instance_activity_dates_quiz_assign() {
        ...
        $this->setUser($student);
        $quizdates = \theme_cass\activity::instance_activity_dates($course->id, $cm, '', '');
        ...
        $dates1 = \theme_cass\activity::instance_activity_dates($course->id, $cm, '', '');
        $dates2 = \theme_cass\activity::instance_activity_dates($course->id, $cm2, '', '');
        ...
        $this->setUser($student);
        $assigndates = \theme_cass\activity::instance_activity_dates($course->id, $cm, '', '');
