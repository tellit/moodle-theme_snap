...
class course {
    ...
    public function setcoverimage(\context $context, $data, $filename) {
        ...
        if (!in_array($ext, local::supported_coverimage_types())) {
            return ['success' => false, 'warning' => get_string('unsupportedcoverimagetype', 'theme_cass', $ext)];
        }
    ...
    public function favorites($userid = null, $fromcache = true) {
        ...
        if (!isset($favorites[$userid])) {
            $favorites[$userid] = $DB->get_records('theme_cass_course_favorites',
                ['userid' => $userid],
                'courseid ASC',
                'courseid'
            );
    ...
    public function setfavorite($courseshortname, $on = true, $userid = null) {
        ...
        if ($on) {
            if (!$favorited) {
                ...
                $DB->insert_record('theme_cass_course_favorites', $data);
            }
        } else {
            if ($favorited) {
                $select = [
    ...
    public function highlight_section($shortname, $sectionnumber, $highlight) {
        ...
        $actionmodel = new \theme_cass\renderables\course_action_section_highlight($course, $section);
    ...
    public function set_section_visibility($shortname, $sectionnumber, $visible) {
        ...
        $modinfo = get_fast_modinfo($course);
        $section = $modinfo->get_section_info($sectionnumber);
        $actionmodel = new \theme_cass\renderables\course_action_section_visibility($course, $section);
        $toc = new \theme_cass\renderables\course_toc($course);
