    ...
    public function write_setting($data) {
        ...
        if (!empty($data)) {
            ...
            if ($categories === false || $categories == null) {
                return get_string('error:categorycolorinvalidjson', 'theme_cass');
            }
            foreach ($categories as $categoryid => $color) {
                if (!is_numeric($categoryid) || !preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $color)  || $color == '#FFFFFF') {
                    return get_string('error:categorycolorinvalidvalue', 'theme_cass', $categoryid);
                }
                try {
                    \coursecat::get($categoryid);
                } catch (\moodle_exception $e) {
                    return get_string('error:categorynotfound', 'theme_cass', $categoryid);
                }
            }
            $sizeelements = $this->validate_text($data);
            if ($sizeelements !== count((array)$categories)) {
                return get_string('error:duplicatedcategoryids', 'theme_cass');
            }
        } else {
            $data = 0;
        }
